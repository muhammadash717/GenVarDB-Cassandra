...
defaults = {
    'output_tsv': None,
    'email': '30007170100491@sci.asu.edu.eg',
    'api_key': 'ak-dU4yP3KIjY1Ub3GA3XtRLq5DEygirq0r',
    'max_retries': 5,
    'retry_delay': 2,
    'api_delay': 0.1,
    'chunk_size': 1000,
...
output_fieldnames = [
    ...
    'phenotype_combined', 'pathogenicity_classification_combined', 'custom_annotations', 'variant_samples'
]

def vcf2json(vcf_gz_path):
    """
    Convert a compressed VCF file into a list of variant dictionaries with keys: chr, pos, ref, alt, variant_samples (genotype).
    """
    variants = []
    with gzip.open(vcf_gz_path, 'rt') as f:
        ...
        for line in f:
            ...
            if not line.startswith('#'):
                ...
                genotype_str = str(genotypes_dict)
                if len(ref) < 150:
                    for alt_allele in alt.split(','):
                        if len(alt_allele) < 150:
                            variants.append({"chr": chrom, "pos": pos, "ref": ref, "alt": alt_allele, "variant_samples": genotype_str})
    return variants
